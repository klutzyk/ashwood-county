...
public partial class AuthoringStudioHost:Node
{
    ...
    private OptionButton _location=null!,_radius=null!,_category=null!,_subcategory=null!,_gameplay=null!,_selectedGameplay=null!,_loot=null!,_roadType=null!,_roadProfileInspector=null!,_layer=null!;
    private LineEdit _search=null!,_name=null!,_roomA=null!,_roomB=null!;
    private GridContainer _assetGrid=null!;private VBoxContainer _validationEntries=null!;private Label _status=null!,_dirtyStatus=null!,_coordinates=null!,_selectionTitle=null!,_id=null!,_validation=null!;
    ...
    private void BuildLeftPanel(Control root)
    {
        ...
        toolsTab.AddChild(new HSeparator());toolsTab.AddChild(Label("SPLINE ROAD NETWORK","HudHeading"));toolsTab.AddChild(Label("Choose a road profile and width, then place control points.","HudTiny"));HBoxContainer roadRow=new();_roadType=new OptionButton();foreach(RoadProfileDefinition profile in RoadProfiles.All)_roadType.AddItem(profile.Id);_roadType.Selected=3;_roadWidth=CompactNumber("ROAD WIDTH",.2,4,.1,RoadProfiles.All[3].DefaultWidth);_roadType.ItemSelected+=_=>{RoadProfileDefinition profile=RoadProfiles.Get(_roadType.GetItemText(_roadType.Selected));_roadWidth.Value=profile.DefaultWidth;ApplyPathSettings();};_roadWidth.ValueChanged+=_=>ApplyPathSettings();roadRow.AddChild(_roadType);roadRow.AddChild(_roadWidth);toolsTab.AddChild(roadRow);toolsTab.AddChild(Button("START DRAWING ROAD",ActivateSelectedRoad));_roadDebug=new CheckBox{Text="SHOW ROAD GRAPH / SPLINE DEBUG"};_roadDebug.Toggled+=_canvas.ToggleRoadDebug;toolsTab.AddChild(_roadDebug);
    ...
    private void BuildInspector(Control root)
    {
        ...
        _name=Field(box,"NAME");_selectedGameplay=new OptionButton();foreach(string type in new[]{"Decoration","Building","Door","Container","Bed","Scavenge Source","Landmark","Zombie Spawn","Resource"})_selectedGameplay.AddItem(type);box.AddChild(Labeled("GAMEPLAY TYPE",_selectedGameplay));_roadProfileInspector=new OptionButton();foreach(RoadProfileDefinition profile in RoadProfiles.All)_roadProfileInspector.AddItem(profile.Id);box.AddChild(Labeled("ROAD PROFILE",_roadProfileInspector));_x=Number(box,"X",out _xLabel);_y=Number(box,"Y",out _yLabel);_width=Number(box,"WIDTH",out _widthLabel,.05,100);_height=Number(box,"HEIGHT",out _heightLabel,.05,100);_target=Number(box,"VISUAL HEIGHT",out _targetLabel,.01,2000);_visualWidth=Number(box,"VISUAL WIDTH",out _visualWidthLabel,.01,2000);_rotation=Number(box,"ROTATION (DEGREES)",out _,1,360);
    ...
    {
        if(_canvas is null||_roadType is null)return;_canvas.PathType=_roadType.GetItemText(_roadType.Selected);_canvas.PathWidth=(float)_roadWidth.Value;_canvas.PathAssetPath=RoadProfiles.Get(_canvas.PathType).SurfaceTexture;
    }
    ...
    private void RefreshInspector()
    {
        ...
        foreach(Control control in new Control[]{_name,_selectedGameplay,_roadProfileInspector,_x,_y,_width,_height,_target,_visualWidth,_rotation,_anchorX,_anchorY,_roomA,_roomB,_collision,_keepAspect,_loot,_duration})SetFieldVisible(control,selected);
    ...
    {
        SetFieldVisible(_roomA,false);SetFieldVisible(_roomB,false);SetFieldVisible(_loot,false);SetFieldVisible(_duration,false);SetFieldVisible(_collision,false);SetFieldVisible(_keepAspect,false);SetFieldVisible(_selectedGameplay,false);SetFieldVisible(_roadProfileInspector,false);SetFieldVisible(_target,true);SetFieldVisible(_visualWidth,false);SetFieldVisible(_rotation,false);SetFieldVisible(_anchorX,false);SetFieldVisible(_anchorY,false);_targetLabel.Text="VISUAL HEIGHT";
        switch(data)
        {
            ...
            case AuthoredPathData item:int pointIndex=_canvas.SelectedRoadPointIndex;float pointWidth=pointIndex>=0&&pointIndex<item.Points.Count?item.Points[pointIndex].WidthScale:1;SetNumbers(0,0,item.Width,pointWidth,0,0,0);Labels("","","ROAD WIDTH",pointIndex>=0?"SELECTED POINT WIDTH SCALE":"","","");SetFieldVisible(_x,false);SetFieldVisible(_y,false);SetFieldVisible(_height,pointIndex>=0);SetFieldVisible(_target,false);SetFieldVisible(_width,true);SetFieldVisible(_roadProfileInspector,item.LineKind=="Road");SelectText(_roadProfileInspector,RoadProfiles.Get(item.PathType).Id);break;
