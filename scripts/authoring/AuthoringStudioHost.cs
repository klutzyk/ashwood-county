...
public partial class AuthoringStudioHost:Node
{
    ...
    private OptionButton _location=null!,_radius=null!,_category=null!,_subcategory=null!,_gameplay=null!,_selectedGameplay=null!,_loot=null!,_roadType=null!,_roadFlavor=null!,_roadProfileInspector=null!,_roadFlavorInspector=null!,_layer=null!,_paintMode=null!;
    private LineEdit _search=null!,_name=null!,_roomA=null!,_roomB=null!;
    private GridContainer _assetGrid=null!,_paintGrid=null!;private VBoxContainer _validationEntries=null!;private Label _status=null!,_dirtyStatus=null!,_coordinates=null!,_selectionTitle=null!,_id=null!,_validation=null!,_paintSummary=null!;private TextureRect _roadPreview=null!;
    ...
    private CheckBox _collision=null!,_snap=null!,_keepAspect=null!,_randomBrush=null!,_roadDebug=null!,_layerVisible=null!,_layerLocked=null!,_cleanupFlag=null!;private Button _editInterior=null!,_exitInterior=null!,_apply=null!,_extendRoad=null!,_splitRoad=null!;private StudioCountyMinimap _minimap=null!;private AssetInspectionPreview _assetPreview=null!;private Label _assetDetails=null!;private AuthoringAssetEntry? _chosenAsset;
    private CountyLocationDefinition[] _locations=[];private readonly Dictionary<string,bool> _layerVisibility=[];private readonly Dictionary<string,bool> _layerLocks=[];private readonly HashSet<string> _cleanupFlags=[];private readonly HashSet<string> _paintSelections=[];private bool _refreshingInspector,_quitArmed,_refreshingLayer,_refreshingAssetQa;
    ...
    public override void _Ready()
    {
        ...
        else if(System.Environment.GetEnvironmentVariable("ASHWOOD_VALIDATE_ROAD_NETWORK")=="1")RunRoadNetworkValidation();
    ...
    private void BuildLeftPanel(Control root)
    {
        ...
        toolsTab.AddChild(new HSeparator());toolsTab.AddChild(Label("TERRAIN / SCATTER BRUSH","HudHeading"));toolsTab.AddChild(Label("Radius: brush size   Density: amount","HudTiny"));toolsTab.AddChild(Label("Variation: random size difference","HudTiny"));HBoxContainer brushRow=new();_brushRadius=CompactNumber("RADIUS",.5,12,.5,2.5);_brushDensity=CompactNumber("DENSITY",.1,2,.1,.65);_brushVariation=CompactNumber("SCALE ±",0,.75,.05,.18);brushRow.AddChild(_brushRadius);brushRow.AddChild(_brushDensity);brushRow.AddChild(_brushVariation);toolsTab.AddChild(brushRow);_randomBrush=new CheckBox{Text="RANDOM ASSET / ROTATION",ButtonPressed=true};_randomBrush.Toggled+=_=>ApplyBrushSettings();toolsTab.AddChild(_randomBrush);_brushRadius.ValueChanged+=_=>ApplyBrushSettings();_brushDensity.ValueChanged+=_=>ApplyBrushSettings();_brushVariation.ValueChanged+=_=>ApplyBrushSettings();ApplyBrushSettings();
        VBoxContainer roadTab=new(){Name="ROADS",SizeFlagsVertical=Control.SizeFlags.ExpandFill};tabs.AddChild(roadTab);roadTab.AddChild(Label("SPLINE ROAD NETWORK","HudHeading"));roadTab.AddChild(Label("Choose a profile, visual flavor and width.","HudTiny"));_roadPreview=new TextureRect{CustomMinimumSize=new Vector2(270,150),ExpandMode=TextureRect.ExpandModeEnum.IgnoreSize,StretchMode=TextureRect.StretchModeEnum.KeepAspectCentered};roadTab.AddChild(_roadPreview);HBoxContainer roadRow=new();_roadType=new OptionButton();foreach(RoadProfileDefinition profile in RoadProfiles.All)_roadType.AddItem(profile.Id);_roadType.Selected=3;_roadFlavor=new OptionButton();_roadWidth=CompactNumber("W",.2,4,.1,RoadProfiles.All[3].DefaultWidth);_roadType.ItemSelected+=_=>{RoadProfileDefinition profile=RoadProfiles.Get(_roadType.GetItemText(_roadType.Selected));_roadWidth.Value=profile.DefaultWidth;RefreshRoadFlavors();};_roadFlavor.ItemSelected+=_=>{RefreshRoadPreview();ApplyPathSettings();};_roadWidth.ValueChanged+=_=>ApplyPathSettings();roadRow.AddChild(_roadType);roadRow.AddChild(_roadWidth);roadTab.AddChild(roadRow);roadTab.AddChild(Label("ROAD FLAVOR / SOURCE ART","HudTiny"));roadTab.AddChild(_roadFlavor);roadTab.AddChild(Button("START DRAWING ROAD",ActivateSelectedRoad));_roadDebug=new CheckBox{Text="SHOW ROAD SPLINE DEBUG"};_roadDebug.Toggled+=_canvas.ToggleRoadDebug;roadTab.AddChild(_roadDebug);RefreshRoadFlavors();

        _paintMode=new OptionButton();foreach(string mode in new[]{"Terrain","Vegetation","Environmental Props"})_paintMode.AddItem(mode);_paintMode.ItemSelected+=_=>RefreshPaintPalette();toolsTab.AddChild(_paintMode);toolsTab.AddChild(Label("Checked thumbnails are the only assets this brush uses.","HudTiny"));ScrollContainer paintScroll=new(){CustomMinimumSize=new Vector2(270,170),SizeFlagsVertical=Control.SizeFlags.ExpandFill,HorizontalScrollMode=ScrollContainer.ScrollMode.Disabled,MouseFilter=Control.MouseFilterEnum.Stop};paintScroll.GuiInput+=input=>{if(input is InputEventMouseButton mouse&&mouse.Pressed&&mouse.ButtonIndex is MouseButton.WheelUp or MouseButton.WheelDown){paintScroll.ScrollVertical+=mouse.ButtonIndex==MouseButton.WheelDown?72:-72;paintScroll.AcceptEvent();}};_paintGrid=new GridContainer{Columns=2,SizeFlagsHorizontal=Control.SizeFlags.ExpandFill};paintScroll.AddChild(_paintGrid);toolsTab.AddChild(paintScroll);_paintSummary=Label("No paint assets checked.","HudTiny");toolsTab.AddChild(_paintSummary);HBoxContainer paintActions=new();paintActions.AddChild(Button("PAINT CHECKED",ActivatePaintPalette));paintActions.AddChild(Button("ERASE",()=>_canvas.SetTool(AuthoringTool.Erase)));toolsTab.AddChild(paintActions);RefreshPaintPalette();
    ...
    private void BuildInspector(Control root)
    {
        ...
        _name=Field(box,"NAME");_selectedGameplay=new OptionButton();foreach(string type in new[]{"Decoration","Building","Door","Container","Bed","Scavenge Source","Landmark","Zombie Spawn","Resource"})_selectedGameplay.AddItem(type);box.AddChild(Labeled("GAMEPLAY TYPE",_selectedGameplay));_roadProfileInspector=new OptionButton();foreach(RoadProfileDefinition profile in RoadProfiles.All)_roadProfileInspector.AddItem(profile.Id);_roadProfileInspector.ItemSelected+=_=>PopulateInspectorFlavors(null);box.AddChild(Labeled("ROAD PROFILE",_roadProfileInspector));_roadFlavorInspector=new OptionButton();box.AddChild(Labeled("ROAD FLAVOR",_roadFlavorInspector));_x=Number(box,"X",out _xLabel);_y=Number(box,"Y",out _yLabel);_width=Number(box,"WIDTH",out _widthLabel,.05,100);_height=Number(box,"HEIGHT",out _heightLabel,.05,100);_target=Number(box,"VISUAL HEIGHT",out _targetLabel,.01,2000);_visualWidth=Number(box,"VISUAL WIDTH",out _visualWidthLabel,.01,2000);_rotation=Number(box,"GROUND FACING (0 / 180)",out _,180,180);
        ...
        _apply=Button("APPLY CHANGES",ApplyInspector);box.AddChild(_apply);_extendRoad=Button("EXTEND SELECTED ROAD",_canvas.ExtendSelectedRoad);box.AddChild(_extendRoad);_splitRoad=Button("SPLIT AT SELECTED POINT",_canvas.SplitSelectedRoad);box.AddChild(_splitRoad);
        ...
        _validation=Label("Select a building and enter Interior Edit.","HudMuted");_validation.AutowrapMode=TextServer.AutowrapMode.WordSmart;box.AddChild(_validation);_validationEntries=new VBoxContainer();box.AddChild(_validationEntries);
    ...
    private IEnumerable<AuthoringAssetEntry> PaintCandidates()
    {
        string mode=_paintMode?.Selected>=0?_paintMode.GetItemText(_paintMode.Selected):"Terrain";
        return mode switch{"Terrain"=>_assets.Where(asset=>asset.Category=="Terrain"),"Vegetation"=>_assets.Where(asset=>asset.Category=="Environment"),_=>_assets.Where(asset=>asset.Category is "Props" or "Waterside" or "Railway" or "Resources")};
    }
    private void RefreshPaintPalette()
    {
        if(_paintGrid is null)return;foreach(Node child in _paintGrid.GetChildren())child.QueueFree();AuthoringAssetEntry[] candidates=PaintCandidates().Take(80).ToArray();HashSet<string> allowed=candidates.Select(item=>item.Path).ToHashSet();_paintSelections.RemoveWhere(path=>!allowed.Contains(path));
        foreach(AuthoringAssetEntry asset in candidates){VBoxContainer tile=new(){CustomMinimumSize=new Vector2(125,100)};Button image=new(){CustomMinimumSize=new Vector2(125,66),Icon=AuthoringThumbnailCache.Get(asset.Path),ExpandIcon=true,TooltipText=$"{asset.Name}\nClick image or checkbox to toggle"};image.Pressed+=()=>TogglePaintAsset(asset,!_paintSelections.Contains(asset.Path));tile.AddChild(image);CheckBox check=new(){Text=Short(asset.Name,16),ButtonPressed=_paintSelections.Contains(asset.Path),TooltipText=asset.Name};check.Toggled+=enabled=>TogglePaintAsset(asset,enabled);tile.AddChild(check);_paintGrid.AddChild(tile);}_paintSummary.Text=_paintSelections.Count==0?"No paint assets checked.":$"✓ {_paintSelections.Count} checked asset(s)";
    }
    private void TogglePaintAsset(AuthoringAssetEntry asset,bool enabled)
    {
        if(enabled)_paintSelections.Add(asset.Path);else _paintSelections.Remove(asset.Path);RefreshPaintPalette();
    }
    private void ActivatePaintPalette()
    {
        AuthoringAssetEntry[] selected=PaintCandidates().Where(asset=>_paintSelections.Contains(asset.Path)).ToArray();if(selected.Length==0){SetStatus("Check at least one paint thumbnail first.");return;}ApplyBrushSettings();_chosenAsset=selected[0];if(_paintMode.GetItemText(_paintMode.Selected)=="Terrain")_canvas.SetTerrainBrush(selected[0],selected);else _canvas.SetScatterBrush(selected[0],selected);SetStatus($"PAINT — using {selected.Length} checked asset(s)");
    }
    ...
    {
        if(_canvas is null||_roadType is null)return;_canvas.PathType=_roadType.GetItemText(_roadType.Selected);_canvas.PathWidth=(float)_roadWidth.Value;RoadFlavorDefinition flavor=RoadFlavors.Get(_canvas.PathType,_roadFlavor?.Selected>=0?_roadFlavor.GetItemText(_roadFlavor.Selected):string.Empty);_canvas.PathFlavor=flavor.Id;_canvas.PathAssetPath=flavor.SurfaceTexture;
    }
    private void RefreshRoadFlavors(){if(_roadFlavor is null)return;_roadFlavor.Clear();foreach(RoadFlavorDefinition flavor in RoadFlavors.ForProfile(_roadType.GetItemText(_roadType.Selected)))_roadFlavor.AddItem(flavor.Id);_roadFlavor.Selected=0;RefreshRoadPreview();ApplyPathSettings();}
    private void RefreshRoadPreview(){if(_roadPreview is null||_roadFlavor is null||_roadFlavor.Selected<0)return;RoadFlavorDefinition flavor=RoadFlavors.Get(_roadType.GetItemText(_roadType.Selected),_roadFlavor.GetItemText(_roadFlavor.Selected));_roadPreview.Texture=ResourceLoader.Exists(flavor.PreviewAsset)?AuthoringThumbnailCache.Get(flavor.PreviewAsset):null;_roadPreview.TooltipText=$"{flavor.Id}\n{flavor.PreviewAsset}";}
    private void PopulateInspectorFlavors(string? selected){if(_roadFlavorInspector is null||_roadProfileInspector.Selected<0)return;_roadFlavorInspector.Clear();foreach(RoadFlavorDefinition flavor in RoadFlavors.ForProfile(_roadProfileInspector.GetItemText(_roadProfileInspector.Selected)))_roadFlavorInspector.AddItem(flavor.Id);SelectText(_roadFlavorInspector,RoadFlavors.Get(_roadProfileInspector.GetItemText(_roadProfileInspector.Selected),selected).Id);}
    private static string DefaultRoadAsset(string style)=>style switch
    {
        "Highway"=>"res://assets/art/terrain/roads/asphalt_straight_02.png",
        "Paved Town Road"=>"res://assets/art/terrain/roads/asphalt_cracked_01.png",
        "Rural Road"=>"res://assets/art/terrain/roads/gravel_road_01.png",
        "Dirt Road"=>"res://assets/art/terrain/roads/dirt_straight_03.png",
        "Farm Track"=>"res://assets/art/terrain/roads/dirt_track_01.png",
        "Forest Track"=>"res://assets/art/terrain/roads/forest_track_01.png",
        _=>"res://assets/art/terrain/roads/rural_path_grass_01.png"
    };
    ...
    private void RefreshInspector()
    {
        ...
        foreach(Control control in new Control[]{_name,_selectedGameplay,_roadProfileInspector,_roadFlavorInspector,_x,_y,_width,_height,_target,_visualWidth,_rotation,_anchorX,_anchorY,_roomA,_roomB,_collision,_keepAspect,_loot,_duration})SetFieldVisible(control,selected);
        _extendRoad.Visible=data is AuthoredPathData { LineKind:"Road" };_splitRoad.Visible=_extendRoad.Visible;
    ...
    {
        SetFieldVisible(_roomA,false);SetFieldVisible(_roomB,false);SetFieldVisible(_loot,false);SetFieldVisible(_duration,false);SetFieldVisible(_collision,false);SetFieldVisible(_keepAspect,false);SetFieldVisible(_selectedGameplay,false);SetFieldVisible(_roadProfileInspector,false);SetFieldVisible(_roadFlavorInspector,false);SetFieldVisible(_target,true);SetFieldVisible(_visualWidth,false);SetFieldVisible(_rotation,false);SetFieldVisible(_anchorX,false);SetFieldVisible(_anchorY,false);_targetLabel.Text="VISUAL HEIGHT";
        switch(data)
        {
            ...
            case AuthoredPathData item:int pointIndex=_canvas.SelectedRoadPointIndex;float pointWidth=pointIndex>=0&&pointIndex<item.Points.Count?item.Points[pointIndex].WidthScale:1;SetNumbers(0,0,item.Width,pointWidth,0,0,0);Labels("","","ROAD WIDTH",pointIndex>=0?"SELECTED POINT WIDTH SCALE":"","","");SetFieldVisible(_x,false);SetFieldVisible(_y,false);SetFieldVisible(_height,pointIndex>=0);SetFieldVisible(_target,false);SetFieldVisible(_width,true);SetFieldVisible(_roadProfileInspector,item.LineKind=="Road");SetFieldVisible(_roadFlavorInspector,item.LineKind=="Road");SelectText(_roadProfileInspector,RoadProfiles.Get(item.PathType).Id);PopulateInspectorFlavors(item.Flavor);break;
    ...
    private void ApplyInspector()
    {
        ...
        switch(data)
        {
            ...
            case AuthoredPathData item:item.Width=Mathf.Max(.15f,(float)_width.Value);if(_canvas.SelectedRoadPointIndex>=0&&_canvas.SelectedRoadPointIndex<item.Points.Count)item.Points[_canvas.SelectedRoadPointIndex].WidthScale=Mathf.Clamp((float)_height.Value,.35f,2.5f);if(item.LineKind=="Road"){item.PathType=_roadProfileInspector.GetItemText(_roadProfileInspector.Selected);item.Flavor=_roadFlavorInspector.GetItemText(_roadFlavorInspector.Selected);item.AssetPath=RoadFlavors.Get(item.PathType,item.Flavor).SurfaceTexture;}break;
    ...
    private void RunValidation(){IReadOnlyList<AuthoringValidationIssue> issues=_canvas.ValidateCurrent();_validation.Text=$"{issues.Count} validation result(s). Click one to focus it.";foreach(Node child in _validationEntries.GetChildren())child.QueueFree();foreach(AuthoringValidationIssue issue in issues.Take(18)){string color=issue.Severity==AuthoringValidationSeverity.Invalid?"RED":issue.Severity==AuthoringValidationSeverity.Warning?"YELLOW":"GREEN";Button entry=Button($"{color}: {issue.Message}",()=>{if(!_canvas.IsInteriorMode)LoadArea(issue.Position,RadiusValue());else{_camera.CenterOnGridPosition(issue.Position);_camera.SetZoom(.84f);}SetStatus(issue.Message);});entry.Alignment=HorizontalAlignment.Left;entry.TooltipText=issue.Message;_validationEntries.AddChild(entry);}}
    ...
    private async void RunRoadNetworkValidation()
    {
        for(int i=0;i<8;i++)await ToSignal(GetTree(),SceneTree.SignalName.ProcessFrame);LoadArea(new Vector2(226,159),1);_camera.SetZoom(.42f);
        AuthoredCountyDocument test=new(){FormatVersion=3,Paths=
        [
            new(){Id="test_highway",DisplayName="Curved Highway",PathType="Highway",Width=2.8f,VariationSeed=101,Points=[AuthoredPointData.From(new(207,151)),AuthoredPointData.From(new(214,149)),AuthoredPointData.From(new(222,154)),AuthoredPointData.From(new(230,156))]},
            new(){Id="test_town",DisplayName="Town Cross Street",PathType="Town Road",Width=1.8f,VariationSeed=202,Points=[AuthoredPointData.From(new(220,145)),AuthoredPointData.From(new(220,164))]},
            new(){Id="test_dirt",DisplayName="Bridge Approach",PathType="Dirt Road",Width=1.35f,VariationSeed=303,Points=[AuthoredPointData.From(new(210,164)),AuthoredPointData.From(new(217,159)),AuthoredPointData.From(new(222,154))]},
            new(){Id="test_bridge_road",DisplayName="Bridge Socket Road",PathType="County Road",Width=2.2f,VariationSeed=404,Points=[AuthoredPointData.From(new(230,156)),AuthoredPointData.From(new(232.02f,157.98f)),AuthoredPointData.From(new(233.08f,156.92f))]},
            new(){Id="test_rural",DisplayName="Rural Asphalt",PathType="Rural Asphalt Road",Width=1.55f,VariationSeed=505,Points=[AuthoredPointData.From(new(204,168)),AuthoredPointData.From(new(211,167)),AuthoredPointData.From(new(218,170))]},
            new(){Id="test_farm",DisplayName="Farm Track",PathType="Farm Track",Width=.95f,VariationSeed=606,Points=[AuthoredPointData.From(new(205,172)),AuthoredPointData.From(new(212,170.5f)),AuthoredPointData.From(new(219,173))]},
            new(){Id="test_logging",DisplayName="Logging Road",PathType="Logging Road",Width=1.45f,VariationSeed=707,Points=[AuthoredPointData.From(new(206,176)),AuthoredPointData.From(new(213,173)),AuthoredPointData.From(new(220,176))]},
            new(){Id="test_footpath",DisplayName="Footpath",PathType="Footpath",Width=.42f,VariationSeed=808,Points=[AuthoredPointData.From(new(207,180)),AuthoredPointData.From(new(214,176)),AuthoredPointData.From(new(221,180))]}
        ],WorldObjects=[new(){Id="test_bridge",DisplayName="County Bridge",AssetPath="res://assets/art/bridges/county_concrete.png",Category="Bridges",GameplayType="Decoration",X=234,Y=156,Scale=.45f,RotationDegrees=0}]};
        _canvas.UseTransientValidationDocument(test);System.Diagnostics.Stopwatch timer=System.Diagnostics.Stopwatch.StartNew();_landscape.RefreshDocument(test);_canvas.ToggleRoadDebug(true);RoadNetworkGraph graph=RoadNetworkGraph.Build(test);foreach(AuthoredPathData road in test.Paths)RoadSplineGeometry.Sample(road);timer.Stop();string serialized=AuthoredContentRepository.Serialize(test);AuthoredCountyDocument restored=AuthoredContentRepository.Deserialize(serialized);bool deterministic=RoadSplineGeometry.Sample(restored.Paths[0]).Count==RoadSplineGeometry.Sample(test.Paths[0]).Count;bool crossing=graph.Nodes.Any(node=>node.Degree>=3);bool bridgeSocket=graph.Nodes.Any(node=>node.BridgeSocket&&node.Degree>0);bool legacy=RoadProfiles.Get("Rural Road").Id=="Rural Asphalt Road";bool profileCoverage=test.Paths.Select(road=>RoadProfiles.Get(road.PathType).Id).Distinct().Count()==RoadProfiles.All.Count;AuthoredPathData editPath=restored.Paths[0];Vector2 beforeMove=RoadSplineGeometry.Sample(editPath)[10].GridPosition;editPath.Points[1].X+=1;Vector2 afterMove=RoadSplineGeometry.Sample(editPath)[10].GridPosition;bool movedRegenerates=beforeMove.DistanceTo(afterMove)>.01f;int beforePoints=editPath.Points.Count;int inserted=RoadSplineGeometry.InsertControlPoint(editPath,(editPath.Points[0].Vector+editPath.Points[1].Vector)*.5f);bool editOps=inserted>0&&editPath.Points.Count==beforePoints+1&&RoadSplineGeometry.RemoveControlPoint(editPath,inserted)&&editPath.Points.Count==beforePoints;IReadOnlyList<AuthoringValidationIssue> issues=AuthoringValidation.ValidateWorld(test);int invalid=issues.Count(issue=>issue.Severity==AuthoringValidationSeverity.Invalid);GD.Print($"ROAD_NETWORK_VALIDATION: {(deterministic&&crossing&&bridgeSocket&&legacy&&profileCoverage&&movedRegenerates&&editOps&&invalid==0?"PASS":"FAIL")} (profiles={RoadProfiles.All.Count}, coverage={profileCoverage}, nodes={graph.Nodes.Count}, edges={graph.Edges.Count}, crossing={crossing}, bridge_socket={bridgeSocket}, deterministic={deterministic}, move_regenerates={movedRegenerates}, edit_ops={editOps}, legacy={legacy}, invalid={invalid}, rebuild_ms={timer.Elapsed.TotalMilliseconds:0.0})");
        string? path=System.Environment.GetEnvironmentVariable("ASHWOOD_STUDIO_CAPTURE_PNG");if(!string.IsNullOrWhiteSpace(path))await SaveCapture(path);GetTree().Quit();
    }

    private async void CaptureStudio(){string? path=System.Environment.GetEnvironmentVariable("ASHWOOD_STUDIO_CAPTURE_PNG");if(FindChild("AuthoringTabs",true,false) is TabContainer tabs)tabs.CurrentTab=path?.Contains("road",StringComparison.OrdinalIgnoreCase)==true?3:2;if(path?.Contains("paint-multi",StringComparison.OrdinalIgnoreCase)==true){foreach(AuthoringAssetEntry asset in PaintCandidates().Take(3))_paintSelections.Add(asset.Path);RefreshPaintPalette();}if(!string.IsNullOrWhiteSpace(path))await SaveCapture(path);}
    ...
    private static SpinBox CompactNumber(string tooltip,double min,double max,double step,double value){string prefix=tooltip=="RADIUS"?"Radius ":tooltip=="DENSITY"?"Density ":tooltip=="SCALE ±"?"Var ±":"Width ";return new(){TooltipText=tooltip,Prefix=prefix,MinValue=min,MaxValue=max,Step=step,Value=value,CustomMinimumSize=new Vector2(tooltip=="ROAD WIDTH"?104:84,0),AllowGreater=false,AllowLesser=false};}
