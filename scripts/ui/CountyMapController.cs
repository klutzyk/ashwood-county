...
public partial class CountyMapController : CanvasLayer
{
    ...
    private void ShowRegion(int i){var r=RegionCatalog.All[i];string status=r.Availability switch{RegionAvailability.Current=>"CURRENT • CONTESTED",RegionAvailability.Known=>"KNOWN • INFESTED",_=>"UNKNOWN"};_detail.Text=$"{r.Name}\n{r.Description}\n\n{r.Environment}  •  Danger {r.Danger}/5\nStatus: {status}\n\nResources\n{string.Join("  •  ",r.Resources)}\n\nLandmarks\n{string.Join("\n",r.Landmarks)}\n\nConnected routes\n{string.Join("  •  ",r.Neighbors)}";}
}
