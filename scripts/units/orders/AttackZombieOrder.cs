...
public sealed class AttackZombieOrder(Zombie target):ISurvivorOrder
{
    ...
    public void Tick(Survivor survivor,double delta)
    {
        ...
        _winding=false;_cooldown=.85f;target.TakeDamage(survivor.EffectiveMeleeDamage,survivor);(survivor.GetTree().GetFirstNodeInGroup(NoiseSystem.GroupName) as NoiseSystem)?.Emit(survivor.SimulationPosition,4.5f);
    }
